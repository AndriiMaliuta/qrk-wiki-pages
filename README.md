# qrk-wiki-pages Project

```
vertx-reactive:postgresql://localhost:5432/mydatabase
```
...
grpcurl -d '{"id": 147}' -plaintext localhost:9000 pages.PagesGrpc/GetPage
```
...
$do$
    ...
    begin
        for i in 3..10 loop
                insert into pages (id, title, body, space_key, author_id, created_at, last_updated, parent_id)
                values (i, 'page ' || i, 'lorem aaa ' || i, 'DEV', 1, now(), now(), 100);
            end loop;
    end;
$do$;
```
...
You can then execute your native executable with: `./target/qrk-wiki-pages-1.0-SNAPSHOT-runner`
